...
def create_slicing_charts(df):
    ...
    # Slicing 1: Average payroll employment by year (custom interval)
    st.subheader("Average Jobs Created (Select Year Range)")

    # Slider to select the year interval for analysis
    ...
    # --- Presidents Section ---
    # List of US presidents and their terms (add or edit as needed)
    presidents = [
    ...
    # Find presidents in the selected interval
    selected_presidents = [
    ...
    # Show presidents in a text input (editable, responsive to slider)
    presidents_text = ", ".join(selected_presidents) if selected_presidents else "No presidents found for this interval."
    st.text_input(
        f"Presidents of the USA during {year_range[0]}–{year_range[1]}:",
        value=presidents_text,
        key="presidents_interval"
    )

    # Filter data for the selected interval and calculate the average
    df_avg_jobs = df[(df['date'].dt.year >= year_range[0]) & (df['date'].dt.year <= year_range[1])]
    avg_jobs_created = df_avg_jobs['total_nonfarm'].mean()

    # Display the result as a single prominent number
    st.metric(
        label=f"Average Jobs Created ({year_range[0]}–{year_range[1]})",
        value=f"{avg_jobs_created:,.0f}"
    )
    ...
    # Calculate average total payroll employment for each year from 2010 to 2025
    df['year'] = df['date'].dt.year
    avg_by_year = (
        df[(df['year'] >= 2010) & (df['year'] <= 2025)]
        .groupby('year')['total_nonfarm']
        .mean()
        .reset_index()
        .rename(columns={'total_nonfarm': 'Average Total Payroll Employment'})
    )
    print(avg_by_year)

    # For average by year
    st.subheader("Average Total Payroll Employment by Year (2010–2025)")
    st.dataframe(avg_by_year)

    # For monthly employment comparison
    st.subheader("Monthly Employment: March–December 2020 vs. 2019")
    st.dataframe(df_slice2[['date', 'year', 'total_nonfarm']])
...
def create_dicing_charts(df):
    ...
    key_events = {
        ...
        (2010, 'Q2'): "Steady job growth resumes.",
        (2020, 'Q1'): "COVID-19 pandemic begins, initial economic shock.",
        (2020, 'Q2'): "Historic job losses due to lockdowns.",
        (2020, 'Q3'): "Partial recovery as restrictions ease.",
        (2020, 'Q4'): "Vaccine news, gradual improvement.",
        (2021, 'Q1'): "Vaccination rollout, economic reopening.",
        (2022, 'Q2'): "Strong labor market, inflation concerns.",
        (2023, 'Q3'): "Labor market stabilizes, tech layoffs.",
        (2025, 'Q4'): "Projected steady growth.",
        # Add more events for other years/quarters as needed
    }

    # Build dynamic event text for selected years and quarter
    events_text = ""
    for yr in range(year_range[0], year_range[1] + 1):
        event = key_events.get((yr, quarter))
        if event:
            events_text += f"<b style='color:#111'>{yr} {quarter}:</b> {event}<br>"

    if events_text:
        st.markdown(
            f"<div style='background:#d4f7d4;padding:16px 18px 12px 18px;border-radius:10px;margin-bottom:12px;'>"
            f"{events_text}</div>",
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            "<div style='background:#d4f7d4;padding:16px 18px 12px 18px;border-radius:10px;margin-bottom:12px;'>"
            "<b style='color:#111'>No key events found for the selected interval and quarter.</b></div>",
            unsafe_allow_html=True
        )
...
def create_roll_up_charts(df):
    ...
    # Roll-up 3: Total Jobs Created Per Decade (1940–2020)
    st.subheader("Total Jobs Created Per Decade (1940–2020)")

    # Ensure 'year' column exists
    df['year'] = df['date'].dt.year

    # Filter data for decades 1940–2020
    df_decades = df[(df['year'] >= 1940) & (df['year'] <= 2029)].copy()
    df_decades['decade'] = (df_decades['year'] // 10) * 10

    # Aggregate: sum of total jobs created per decade
    decade_agg = df_decades.groupby('decade')['total_nonfarm'].sum().reset_index()

    # Convert decade to string for better x-axis labeling
    decade_agg['decade'] = decade_agg['decade'].astype(str) + "s"

    # Create a color sequence for each bar
    color_sequence = px.colors.qualitative.Set2
    # If there are more decades than colors, repeat the color list
    color_sequence = (color_sequence * ((len(decade_agg) // len(color_sequence)) + 1))[:len(decade_agg)]
    ...
    fig_decade_sum = px.bar(
        ...
        title="Total Jobs Created Per Decade (1940–2020)",
        labels={'total_nonfarm': 'Total Jobs Created (in thousands)', 'decade': 'Decade'},
    )
    fig_decade_sum.update_layout(showlegend=False)
...
def create_subcube_analysis(df):
    st.header("Sub-Cube Analysis")

    # 1. For months where employment dropped more than 2% MoM, how long did it take to return to the prior peak?
    st.subheader("Recovery Time After >2% Monthly Employment Drop")
    df['mom_growth'] = df['total_nonfarm'].pct_change() * 100
    df['month_year'] = df['date'].dt.strftime('%b-%Y')
    drops = df[df['mom_growth'] < -2].copy()
    recovery_data = []
    for idx, row in drops.iterrows():
        drop_date = row['date']
        pre_peak = df[df['date'] < drop_date]['total_nonfarm'].max()
        post_drop = df[df['date'] > drop_date]
        recovery_idx = post_drop[post_drop['total_nonfarm'] >= pre_peak].first_valid_index()
        if recovery_idx:
            months_to_recover = (df.loc[recovery_idx]['date'].year - drop_date.year) * 12 + (df.loc[recovery_idx]['date'].month - drop_date.month)
            recovery_data.append({
                'Drop Month': row['month_year'],
                'Prior Peak Date': df.loc[df[df['date'] < drop_date]['total_nonfarm'].idxmax()]['date'].strftime('%b-%Y'),
                'Months to Recover': months_to_recover
            })
        else:
            recovery_data.append({
                'Drop Month': row['month_year'],
                'Prior Peak Date': 'N/A',
                'Months to Recover': 'Not recovered yet'
            })
    st.dataframe(pd.DataFrame(recovery_data))

    # 2. Within Q4 (Oct–Dec) of each year, which month consistently shows the highest payroll growth?
    st.subheader("Q4: Month with Highest Payroll Growth Each Year")
    df['year'] = df['date'].dt.year
    df['month'] = df['date'].dt.month
    df['month_name'] = df['date'].dt.strftime('%b')
    df['mom_growth'] = df['total_nonfarm'].pct_change() * 100
    q4 = df[df['month'].isin([10, 11, 12])].copy()
    q4['abs_growth'] = q4['total_nonfarm'].diff()
    q4['prev_year'] = q4['year'].shift(1)
    q4['prev_month'] = q4['month'].shift(1)
    # For each year, find the Q4 month with the highest MoM growth
    q4_max = q4.groupby('year').apply(lambda x: x.loc[x['mom_growth'].idxmax()])
    q4_max = q4_max[['year', 'month_name', 'mom_growth']]
    q4_max = q4_max.rename(columns={'month_name': 'Month', 'mom_growth': 'MoM Growth (%)'})
    st.dataframe(q4_max)

    # Optional: Show which month is most frequently the Q4 winner
    freq = q4_max['Month'].value_counts().reset_index()
    freq.columns = ['Month', 'Times Highest in Q4']
    st.bar_chart(freq.set_index('Month'))
...
def main():
    ...
    if data is not None:
        ...
        elif menu_selection == "Sub-Cube":
            create_subcube_analysis(data.copy())
